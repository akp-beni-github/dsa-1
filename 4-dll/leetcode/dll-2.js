...
class DoublyLinkedList {
    ...
    reverse(){
        ...
        let temp= null;
        while(current !== null ){
            temp = current.prev; //point to current.temp since current.prev will changed
            current.prev=current.next; //flip prev arrow
            current.next=temp //flip next 

            current=current.next;//move the iterattion

         }
         //flip thishead thistail
         let temp2=this.head;
         this.head=this.tail;
         this.tail=temp2;
    }
