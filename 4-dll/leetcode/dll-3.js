...
class DoublyLinkedList {
    ...
    // WRITE THE ISPALINDROME METHOD HERE //wether you read from the front or the back its all the same
    isPalindrome(){
        if (this.length<=1){
            return true;
        }
        ...
        for(let i=0; i<this.length/2; i++){
            if (forwardNode.value!== backwardNode.value){//when not match break false
                return false
            }
            //still equal
