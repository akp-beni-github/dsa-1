...
function removeElement(nums, val){
    let i =0;
    for (let j=0; j < nums.length; j++){ //to j = array.length-1
        if (nums[j] !== val) {
            nums[i]= nums[j];
            i++
        }
        else if (nums[j] == val){
            //doing none j will be greater than i
            // and that's the equivalent to shifting everything else than the value to the front
        }
