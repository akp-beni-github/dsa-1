...
function removeDuplicates(array){ // constrain is that The input array is sorted in ascending order.
    ...
    for (let readpointer =1; readpointer < array.length; readpointer++) {
        if (array[readpointer] !== array[readpointer-1]){
            array[writepointer] = array[readpointer];// set prev = next
            writepointer++;

        }
    }array.slice(0, writepointer);
    return writepointer;
