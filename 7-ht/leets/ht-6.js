...
//fail at few cases because of complement

function subarraySum(nums, target){//two sum but thenum is keep++ and {[currentsummation, currentidx],[nextsum, nextidx]}
    const sumIndex = new Map();
    let currentsum = 0;
    sumIndex.set(currentsum, -1)//initialize [0,-1], [x, 0], [y, 1], ....

    for (let idx=0; idx<nums.length; idx++ ){
        currentsum = currentsum + nums[idx];//+value of idx
        let complement = target - currentsum;  
        if (sumIndex.has(complement)){
            //return [sumIndex.get(complement), idx];
            return [sumIndex.get(complement)+1, idx];
        }
        //set
        sumIndex.set(currentsum, idx);//sum,idx
    }return [];
}
